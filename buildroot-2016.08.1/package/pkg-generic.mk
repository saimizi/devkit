...
# Unpack the archive
# some packages have messed up permissions inside, so chmod is needed.
# with gitj method, package is directly cloned into build directory, no need to make/extract a tarball
$(BUILD_DIR)/%/.stamp_extracted:
	$(call step_start,extract)
	$(call MESSAGE,"Extracting")
	$(foreach hook,$($(PKG)_PRE_EXTRACT_HOOKS),$(call $(hook))$(sep))
	$(if $(filter $($(PKG)_SITE_METHOD),gitj),(true), \
		(\
		  (mkdir -p $(@D)) && \
		  ($(if $($(PKG)_EXTRACT_CMDS),$($(PKG)_EXTRACT_CMDS),true)) && \
		  (chmod -R +rw $(@D)) \
		)\
	)
